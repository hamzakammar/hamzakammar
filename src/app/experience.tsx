...
export default function Experience() {
  return (
    <div className="max-w-2xl ml-8 px-4 py-12 text-white">
      <h1 className="text-2xl font-bold mb-10 text-center text-white">Experience</h1>
      <div className="space-y-8">
        {experiences.map((exp, idx) => (
          <div 
            key={idx} 
            className="relative pl-6 border-l border-blue-400 transition-all duration-300 hover:text-white hover:scale-110 cursor-pointer group"
          >
            <div className="absolute w-3 h-3 bg-blue-500 rounded-full -left-1.5 top-1.5 transition-all duration-300 group-hover:bg-white group-hover:scale-150"></div>
            <div className="mb-1 text-sm text-gray-300 group-hover:text-white">{exp.date}</div>
            <h2 className="text-lg font-semibold text-white group-hover:text-white">{exp.title} @ {exp.company}</h2>
